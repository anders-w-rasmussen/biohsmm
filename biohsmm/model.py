...
class model_1:
    ...
    def train(self, obs, obs_start, num_iterations):
        ...
        for i in range(0, np.size(self.tmat_prior, axis=0)):
            for j in range(0, np.size(self.tmat_prior, axis=1)):
                if np.isinf(self.tmat[i, j]) == True:
                    self.tmat[i, j] = 0
        ...
        for iteration in range(0, num_iterations):
            print("Started Iteration {iter}".format(iter=iteration))

            start_time = time()
            likelihood = calc_likelihood(obs, obs_start, self.states)
            self.tmat, self.states = baum_welch_alg(self.pi_prior, self.tmat_prior, self.pi, self.tmat, self.states,
                                                    likelihood,
                                                    obs)
    ...
    def give_gammas(self, obs, obs_start, state_num):
        likelihood = calc_likelihood(obs, obs_start, self.states)
        ...
        with open(out_directory + "calls.bed", 'w') as f:
            for i in range(0, len(region_list)):
                ...
                if int(region_list[i][1]) - int(region_list[i][0]) >= 8:
                    f.write(str("chr9") + "\t" + str(region_list[i][0]) + "\t" + str(region_list[i][1]) + "\n")

        # FOR SPECIFIC CRISPR SCREEN NEED TO REPLACE
        if write_wig == True:
            with open(out_directory + "gammas.wig", 'w') as f:
                f.write("variableStep chrom=chr9" + "\n")
